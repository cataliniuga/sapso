...
pub fn optimize_hyperparameters(tsp: &TspLib, num_trials: usize) -> Vec<OptimizationResult> {
    ...
    (0..num_trials).into_par_iter().for_each(|_| {
        ...
        let aco_params = AcoParams {
            alpha: rng.gen_range(0.5..4.0),
            beta: rng.gen_range(1.0..5.0),
            decay: rng.gen_range(0.01..0.5),
            q: rng.gen_range(1.0..500.0),
            ants: rng.gen_range(50..500),
            iterations: rng.gen_range(200..2000),
        };
        ...
        let sa_params = SaParams {
            initial_temp: rng.gen_range(1000.0..50000.0),
            final_temp: rng.gen_range(0.0001..0.1),
            cooling_rate: rng.gen_range(0.001..0.3),
        };
        ...
        let ga_params = GaParams {
            population_size: rng.gen_range(100..2000),
            generations: rng.gen_range(100..5000),
            mutation_rate: rng.gen_range(0.001..0.3),
        };
        ...
        let pso_params = PsoParams {
            num_particles: rng.gen_range(50..1000),
            iterations: rng.gen_range(200..5000),
            cognitive_weight: rng.gen_range(0.5..4.0),
            social_weight: rng.gen_range(0.5..4.0),
            inertia_weight: rng.gen_range(0.1..0.9),
        };
