    ...
    T: HeuristicAlgorithm,
{
    ...
    println!("{} best route: {:?}", name, best_route.distance);
    println!("{} run time: {} ms", name, run_time);
    plot::plot_algo_result(&algorithm, name, &plotters::style::BLUE).unwrap();
}
...
fn main() -> Result<()> {
    ...
    let pso = pso::ParticleSwarmOptimization::new(&tsp, 250, 2000, 1.5, 1.5, 0.8, 10);
    run_algorithm(pso, "Particle Swarm Optimization", &tsp, plotters::style::YELLOW);
